...
uint64_t Wrap32::unwrap(Wrap32 zero_point, uint64_t checkpoint) const {
  ...
  auto UnwrapCandidate = [&](const uint32_t period_index) -> uint64_t {
    ...
    const uint64_t unwrap_base{
        raw_value_ < ISN ? kPeriodLength - ISN + raw_value_ : raw_value_ - ISN};
  ...
  const std::array<uint64_t, 3> candidates{
      ...
      UnwrapCandidate(period_index), UnwrapCandidate(period_index + 1)};
  return *std::ranges::min_element(
      candidates, [&](const uint64_t lhs, const uint64_t rhs) {
        return Unsigned64Distance(lhs, checkpoint) <
               Unsigned64Distance(rhs, checkpoint);
      });
}
