...
void TCPReceiver::receive(TCPSenderMessage message) {
  ...
  const uint64_t absolute_sequence_number{sequence_number.unwrap(
      *zero_point_, AbsoluteSequenceNumberOfFirstUnassembled())};
...
TCPReceiverMessage TCPReceiver::send() const {
  const uint64_t absolute_sequence_number{
      AbsoluteSequenceNumberOfFirstUnassembled() + is_finished_};
  const std::optional<Wrap32> ackno =
      zero_point_ ? std::make_optional(
                        Wrap32::wrap(absolute_sequence_number, *zero_point_))
                  : std::nullopt;
